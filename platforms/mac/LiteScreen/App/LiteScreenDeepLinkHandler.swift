//
//  LiteScreenDeepLinkHandler.swift
//  LiteScreen
//
//  Handles litescreen:// automation URLs for external launchers and workflows.
//
...
private enum AppURLScheme {
  #if DEBUG
    static let current = "litescreen-debug"
    static let supported = Set([current, "litescreen"])
  #else
    static let current = "litescreen"
    static let supported = Set([current])
  #endif
}

@MainActor
struct LiteScreenDeepLinkHandler {
  ...
  func handle(_ url: URL) {
    ...
    guard let action = LiteScreenDeepLinkAction(url: url) else {
      DiagnosticLogger.shared.log(
        .warning,
        .action,
...
enum LiteScreenDeepLinkAction: Equatable {
  case captureOneShot
  case openHistory
