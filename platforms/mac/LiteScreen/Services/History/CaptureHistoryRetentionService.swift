...
private let logger = Logger(subsystem: "Lite Screen", category: "CaptureHistoryRetentionService")
...
final class CaptureHistoryRetentionService {
  ...
  func sweep() async {
    ...
    // Collect app-managed file paths before deleting records so we can clean them up afterward.
    ...
    do {
      ...
      if retentionDays > 0 {
        ...
        for record in allRecords where record.capturedAt < cutoff {
          if tempManager.isTempFile(record.fileURL) || MediaClipboardMonitor.isManagedFile(record.fileURL) {
      ...
      if maxCount > 0, allRecords.count > maxCount {
        ...
        for record in oldestRecords {
          if tempManager.isTempFile(record.fileURL) || MediaClipboardMonitor.isManagedFile(record.fileURL),
             !pathsToDelete.contains(record.filePath) {
            pathsToDelete.append(record.filePath)
          }
        }
      }
  ...
  func clearAllHistory() {
    guard CaptureHistoryStore.shared.removeAll() else {
      logger.error("Clear all history skipped because database rows were not removed")
      DiagnosticLogger.shared.log(
        .warning,
        .history,
        "Clear all history skipped; database rows were not removed"
      )
      return
    }
