...
//  LiteScreen
//
//  Core logging engine — appends to daily .txt files in ~/Library/Logs/LiteScreen/
//
...
final class DiagnosticLogger {
  ...
  private let logDirectoryName = "Lite Screen"
  private let filePrefix = "litescreen_"
  private let fileExtension = "txt"

  // MARK: - State

  private let writeQueue = DispatchQueue(label: "com.ahtcfg24.litescreen.diagnosticlogger", qos: .utility)
  ...
  private func writeSessionHeader() {
    ...
    \(osString) | LiteScreen \(appVersion) (\(buildNumber)) | \(cpuArch)
