...
final class QuickAccessActionConfigurationStore: ObservableObject {
  ...
  private static let supportedActions = Set(QuickAccessActionKind.defaultOrder)
  ...
  func setEnabled(_ action: QuickAccessActionKind, enabled: Bool) {
    guard Self.supportedActions.contains(action) else { return }
  ...
  func applyConfiguration(
    ...
    if let enabledActions {
      self.enabledActions = enabledActions.intersection(Self.supportedActions)
    }
  ...
  private static func normalizedEnabledActions(from rawIDs: [String]?) -> Set<QuickAccessActionKind> {
    ...
    return Set(rawIDs.compactMap(QuickAccessActionKind.init(rawValue:))).intersection(supportedActions)
  }
  ...
  private static func normalizedSlotAssignments(
    ...
    for slot in QuickAccessActionSlot.allCases {
      let action: QuickAccessActionKind? = if let rawAction = rawAssignments[slot.rawValue] {
        rawAction.isEmpty ? nil : QuickAccessActionKind(rawValue: rawAction)
      } else {
