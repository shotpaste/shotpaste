...
/// Manages detection and resolution of conflicts between active ShotPaste shortcuts
...
final class SystemScreenshotShortcutManager {
  ...
  func hasConflictingSystemShortcuts() -> Bool {
    ...
    for kind in GlobalShortcutKind.allCases where kind.isSystemConflictRelevant {
      ...
      guard let shotpasteShortcut = KeyboardShortcutManager.shared.shortcut(for: kind) else { continue }

      if !matchingSystemHotkeys(for: kind, shortcut: shotpasteShortcut, in: hotkeys).isEmpty {
        return true
      }
    }
  ...
  /// Return human-readable system shortcut names that currently conflict with a proposed ShotPaste shortcut.
  ...
  private func matchingSystemHotkeys(
    ...
    relevantSystemHotkeys(for: kind).filter { hotkeyID in
      ...
      if matches {
        DiagnosticLogger.shared.log(
          .info, .action,
          "System screenshot hotkey \(hotkeyID.rawValue) matches ShotPaste \(kind.rawValue) shortcut"
        )
      }
      return matches
    }
