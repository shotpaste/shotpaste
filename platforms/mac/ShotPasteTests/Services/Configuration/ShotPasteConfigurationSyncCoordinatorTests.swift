//
//  ShotPasteConfigurationSyncCoordinatorTests.swift
//  ShotPasteTests
//
...
@MainActor
final class ShotPasteConfigurationSyncCoordinatorTests: XCTestCase {
  func testScheduleSyncDebouncesRepeatedRequests() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        syncCount += 1
        return ShotPasteConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  func testSyncNowReportsConflictWithoutConfirmedOverwrite() throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        syncCount += 1
        return ShotPasteConfigurationSyncResult(status: .needsConfirmation, fileURL: fileURL)
      },
  ...
  func testPermissionRequiredStatusIsExposed() throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        ShotPasteConfigurationSyncResult(status: .permissionRequired, fileURL: fileURL)
      }
    )
    defer { coordinator.stop() }
  ...
  func testDefaultsChangeSignatureIsComputedOnceAfterDebounceNotPerRequest() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        ShotPasteConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  private func makeCoordinator(
    ...
    sync: @escaping @MainActor () throws -> ShotPasteConfigurationSyncResult = {
      ShotPasteConfigurationSyncResult(status: .alreadyCurrent, fileURL: URL(fileURLWithPath: "/tmp/config.toml"))
    },
    backgroundSync: (@MainActor () async throws -> ShotPasteConfigurationSyncResult)? = nil,
  ...
  ) -> ShotPasteConfigurationSyncCoordinator {
    ShotPasteConfigurationSyncCoordinator(
      notificationCenter: notificationCenter,
  ...
  private func temporaryConfigURL() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("shotpaste-config-sync-\(UUID().uuidString).toml")
  }
}
