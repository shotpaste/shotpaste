...
final class AppLanguageManager: ObservableObject {
  ...
  func relaunchApplication() async throws {
    ...
    guard Bundle.main.bundleIdentifier == AppBundleIdentity.expected else {
      throw NSError(
        domain: "com.ahtcfg24.litescreen.relaunch",
        code: 1,
        userInfo: [NSLocalizedDescriptionKey: "Refusing to relaunch an unexpected application identity."]
      )
    }
    guard bundleURL.pathExtension.lowercased() == "app" else {
      throw NSError(
        domain: "com.ahtcfg24.litescreen.relaunch",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The current Lite Screen application bundle could not be located."]
      )
    }
