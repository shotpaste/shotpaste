...
final class RecordingAnnotationCanvasView: NSView {
  ...
  private var temporaryClearMode: AnnotationClearMode?
  ...
  override func mouseDown(with event: NSEvent) {
    ...
    isDrawing = true
    temporaryClearMode = state.temporaryClearMode(for: event.modifierFlags)
    drawStart = point
    currentPath = [point]
    needsDisplay = true
  }
  ...
  override func mouseUp(with event: NSEvent) {
    ...
    if let item = RecordingAnnotationFactory.createAnnotation(
      ...
      state.appendAnnotation(
        item,
        tool: state.selectedTool,
        temporaryOverride: temporaryClearMode
      )
    }

    currentPath.removeAll()
    temporaryClearMode = nil
    needsDisplay = true
  }
