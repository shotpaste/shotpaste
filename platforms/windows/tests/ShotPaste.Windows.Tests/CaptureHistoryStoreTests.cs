...
public sealed class CaptureHistoryStoreTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"shotpaste-history-{Guid.NewGuid():N}");
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"shotpaste-history-text-{Guid.NewGuid():N}");
        Directory.CreateDirectory(directory);
        var database = Path.Combine(directory, "history.sqlite3");
        var thumbnails = Path.Combine(directory, "thumbnails");
