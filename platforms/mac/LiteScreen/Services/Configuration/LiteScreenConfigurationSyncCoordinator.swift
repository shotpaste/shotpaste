//
//  LiteScreenConfigurationSyncCoordinator.swift
//  LiteScreen
//
...
@MainActor
final class LiteScreenConfigurationSyncCoordinator: ObservableObject {
  ...
  static let shared = LiteScreenConfigurationSyncCoordinator()

  @Published private(set) var status: Status = .idle

  private let notificationCenter: NotificationCenter
  private let debounceInterval: TimeInterval
  private let syncManagedConfigIfSafe: @MainActor () throws -> LiteScreenConfigurationSyncResult
  private let syncManagedConfigIfSafeInBackground: @MainActor () async throws -> LiteScreenConfigurationSyncResult
  ...
  init(
    ...
    currentSettingsSignature = {
      LiteScreenConfigurationAutoImporter.contentSignature(for: service.exportTOML())
    }
  }
  ...
  convenience init(
    ...
    self.init(
      service: LiteScreenConfigurationService.shared,
  ...
  init(
    ...
    syncManagedConfigIfSafe: @escaping @MainActor () throws -> LiteScreenConfigurationSyncResult,
    syncManagedConfigIfSafeInBackground: (@MainActor () async throws -> LiteScreenConfigurationSyncResult)? = nil,
  ...
  /// `stop()` owns all observer and task cleanup, so destruction has no
  /// actor-bound work once callers have stopped the coordinator.
  nonisolated deinit {}
  ...
  @discardableResult
  func flushPendingSync(reason: Reason) throws -> LiteScreenConfigurationSyncResult? {
  ...
  @discardableResult
  func syncNow(reason: Reason = .manual) throws -> LiteScreenConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
    var syncResult: LiteScreenConfigurationSyncResult?
    defer {
  ...
  @discardableResult
  func syncNowInBackground(reason: Reason = .manual) async throws -> LiteScreenConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
  ...
  private func updateStatus(for result: LiteScreenConfigurationSyncResult, sequence: Int) {
  ...
  private func finishSyncing(after result: LiteScreenConfigurationSyncResult?, sequence _: Int) {
  ...
  private func log(_ result: LiteScreenConfigurationSyncResult, reason: Reason) {
    DiagnosticLogger.shared.log(
      .debug,
      .preferences,
