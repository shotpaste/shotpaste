# AGENTS.md
# 项目
Lite Screen 包含彼此独立的 macOS 与 Windows 原生实现，核心范围为截图、滚动截屏、录屏、截图/剪贴板历史，并要求两个平台保持产品体验对齐。
...
* 发布版本的签名身份固定为自签名证书 `Lite Screen Self-Signed`（指纹 `35517841F1D32EC1ED7D1F411565845C4AA4B70A`，存于 GitHub Secrets `SELF_SIGNED_CERT_P12`/`SELF_SIGNED_CERT_PASSWORD`），详情见 docs/DEVELOPMENT.md 的 "Release signing identity"。macOS 将隐私权限绑定到签名身份，严禁更换、重建或轮换发布证书
