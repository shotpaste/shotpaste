...
final class MicrophoneAudioCapturerTests: XCTestCase {
  ...
  func testMicrophoneAudioCapturerStartStopRealMicrophoneIntegration() throws {
    try XCTSkipUnless(
      ProcessInfo.processInfo.environment["SHOTPASTE_RUN_MICROPHONE_INTEGRATION"] == "1",
      "Real microphone integration is opt-in. Set SHOTPASTE_RUN_MICROPHONE_INTEGRATION=1 to run."
    )
    try XCTSkipUnless(
