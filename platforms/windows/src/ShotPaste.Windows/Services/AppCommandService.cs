...
public sealed class AppCommandService : IDisposable
{
    ...
    public void Start(Action<IReadOnlyList<string>> handler)
    {
        ...
        foreach (var temporary in Directory.EnumerateFiles(_queueDirectory, ".shotpaste-command-*.tmp"))
        {
            try
            {
        ...
        _watcher = new FileSystemWatcher(_queueDirectory, ".shotpaste-command-*.json")
        {
        ...
        foreach (var path in Directory.EnumerateFiles(_queueDirectory, ".shotpaste-command-*.json"))
        ...
        string? instanceScope = null)
    {
        ...
        var temporary = Path.Combine(queueDirectory, $".shotpaste-command-{token}.tmp");
