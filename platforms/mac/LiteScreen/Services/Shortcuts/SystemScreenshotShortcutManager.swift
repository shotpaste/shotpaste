...
/// Manages detection and resolution of conflicts between active LiteScreen shortcuts
...
final class SystemScreenshotShortcutManager {
  ...
  /// Check whether the enabled One Shot shortcut conflicts with a macOS screenshot shortcut.
  ///
  ...
  func hasConflictingSystemShortcuts() -> Bool {
    ...
    for kind in GlobalShortcutKind.allCases where kind.isSystemConflictRelevant {
      ...
      guard let litescreenShortcut = KeyboardShortcutManager.shared.shortcut(for: kind) else { continue }

      if !matchingSystemHotkeys(for: kind, shortcut: litescreenShortcut, in: hotkeys).isEmpty {
        return true
      }
    }
  ...
  /// Return human-readable system shortcut names that currently conflict with a proposed LiteScreen shortcut.
  ...
  private func matchingSystemHotkeys(
    ...
    relevantSystemHotkeys(for: kind).filter { hotkeyID in
      ...
      if matches {
        DiagnosticLogger.shared.log(
          .info, .action,
          "System screenshot hotkey \(hotkeyID.rawValue) matches Lite Screen \(kind.rawValue) shortcut"
        )
      }
      return matches
    }
  ...
  private func relevantSystemHotkeys(for kind: GlobalShortcutKind) -> [SystemHotkeyID] {
    kind == .oneShot ? SystemHotkeyID.allCases : []
  }
