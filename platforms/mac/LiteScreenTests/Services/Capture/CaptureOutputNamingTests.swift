...
final class CaptureOutputNamingTests: XCTestCase {
  ...
  override func setUp() {
    ...
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("LiteScreenTests_CaptureOutputNaming_\(UUID().uuidString)", isDirectory: true)
  ...
  func testResolveBaseName_withNilCustomName_fallsBackToTemplate() {
    ...
    // Default template: "Lite Screen_{datetime}_{ms}"
    XCTAssertTrue(result.hasPrefix("Lite Screen_"), "Expected template-based name, got: \(result)")
  ...
  func testResolveBaseName_withWhitespaceOnlyCustomName_fallsBackToTemplate() {
    ...
    XCTAssertTrue(result.hasPrefix("Lite Screen_"))
  }
  ...
  func testResolveTemplateBaseName_returnsPreviewSubpath() {
    let result = CaptureOutputNaming.resolveTemplateBaseName(
      "Shots/{timestamp}/LiteScreen_{ms}",
      kind: .screenshot,
      date: fixedDate
    )

    XCTAssertEqual(result, "Shots/\(Int(fixedDate.timeIntervalSince1970))/LiteScreen_123")
  }
  ...
  func testCaptureOutputKind_defaultTemplates() {
    XCTAssertEqual(CaptureOutputKind.screenshot.defaultTemplate, "Lite Screen_{datetime}_{ms}")
    XCTAssertEqual(CaptureOutputKind.recording.defaultTemplate, "Lite Screen_Recording_{datetime}")
  }
  ...
  func testResolveBaseName_invalidTemplate_usesFallbackName() {
    ...
    // Fallback format: "Lite Screen_{yyyy-MM-dd_HH-mm-ss-SSS}"
    XCTAssertTrue(result.hasPrefix("Lite Screen_"), "Expected fallback name, got: \(result)")
  }
