...
final class HistoryFloatingPanelController {
  ...
  private var shouldRefocusAfterShow = false
  ...
  /// The panel can briefly resign key while the capture overlay restores the
  /// previously active application. Treat that focus churn as part of the
  /// presentation transition instead of as an outside-click dismissal.
  var isShowing: Bool {
    state == .showing
  }

  func refocusAfterPresentation() {
    guard state == .showing else {
      focusPanel()
      return
    }
    shouldRefocusAfterShow = true
  }
