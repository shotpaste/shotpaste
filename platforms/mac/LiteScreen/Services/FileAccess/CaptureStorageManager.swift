...
//  LiteScreen
//
//  Manages temporary capture storage in Application Support/Lite Screen/Captures.
...
private let logger = Logger(subsystem: "Lite Screen", category: "CaptureStorageManager")
...
final class CaptureStorageManager {
  ...
  private let appSupportFolderName = "Lite Screen"
  ...
  /// URL to the captures cache directory: Application Support/Lite Screen/Captures
  ...
  func clearCache() async throws -> Int {
    ...
    let deletedPaths = await Task.detached(priority: .utility) { () -> [String] in
      ...
      let backgroundLogger = Logger(subsystem: "Lite Screen", category: "CaptureStorageManager")
