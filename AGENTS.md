...
* 发布版本的签名身份固定为 SHA-1 指纹 `35517841F1D32EC1ED7D1F411565845C4AA4B70A` 所对应的既有自签名证书（存于 GitHub Secrets `SELF_SIGNED_CERT_P12`/`SELF_SIGNED_CERT_PASSWORD`），详情见 docs/DEVELOPMENT.md 的 "Release signing identity"。macOS 将隐私权限绑定到签名身份，严禁更换、重建、重命名或轮换发布证书
