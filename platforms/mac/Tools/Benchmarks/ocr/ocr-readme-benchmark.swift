...
struct OCRReadmeBenchmark {
  ...
  private static func benchmarkCorpus() -> [CorpusEntry] {
    [
      ...
      CorpusEntry(
        languageIdentifier: "en",
        text: "Review notes before export so everyone copies the same clean text from the capture."
      ),
      CorpusEntry(
        languageIdentifier: "en",
        text: "Fast OCR helps when release notes, code snippets, and settings panels need to be copied quickly."
      ),
      ...
      CorpusEntry(
        languageIdentifier: "vi",
        text: "Khi cần chia sẻ hướng dẫn nội bộ, OCR chính xác sẽ giảm rất nhiều thời gian chỉnh sửa thủ công."
      ),
      CorpusEntry(
        languageIdentifier: "vi",
      ...
      CorpusEntry(
        languageIdentifier: "es",
        text: "Cuando una pantalla contiene varios párrafos, el OCR debe mantener una lectura natural y coherente."
      ),
      CorpusEntry(
        languageIdentifier: "es",
      ...
      CorpusEntry(
        languageIdentifier: "ru",
        text: "Если на изображении несколько абзацев, OCR должен сохранять естественный порядок чтения."
      ),
      CorpusEntry(
        languageIdentifier: "ru",
        text: "Точные цифры, знаки препинания и подписи важны для внутренних инструкций и релизных заметок."
      ),
      ...
      CorpusEntry(
        languageIdentifier: "fr",
        text: "Quand une image contient plusieurs paragraphes, l’OCR doit préserver une lecture naturelle et stable."
      ),
      CorpusEntry(
        languageIdentifier: "fr",
        text: "Les titres, les chiffres et la ponctuation comptent aussi pour les notes produit et la documentation."
      ),
      ...
      CorpusEntry(
        languageIdentifier: "de",
        text: "Wenn ein Bild mehrere Absätze enthält, sollte die OCR eine natürliche und konsistente Lesereihenfolge behalten."
      ),
      CorpusEntry(
        languageIdentifier: "de",
        text: "Auch Zahlen, Satzzeichen und UI-Bezeichnungen müssen für interne Dokumentation zuverlässig erkannt werden."
      ),
    ]
  }
  ...
  private static func renderImage(text: String, style: RenderStyle) -> CGImage? {
    ...
    attributedText.draw(
      with: NSRect(
        x: textInset,
        y: textInset,
        width: canvasWidth - (textInset * 2),
        height: canvasHeight - (textInset * 2)
      ),
      options: [.usesLineFragmentOrigin, .usesFontLeading]
    )
    image.unlockFocus()
