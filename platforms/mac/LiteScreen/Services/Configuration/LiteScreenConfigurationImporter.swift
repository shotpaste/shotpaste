//
//  LiteScreenConfigurationImporter.swift
//  LiteScreen
//
//  Applies validated TOML config to LiteScreen stores.
//
...
@MainActor
enum LiteScreenConfigurationImporter {
  private struct PreparedImport {
    let issues: [LiteScreenConfigurationIssue]
    let mutations: [() -> Void]
  }

  static func validateTOML(_ source: String, defaults: UserDefaults = .standard) -> [LiteScreenConfigurationIssue] {
    prepareImport(source, defaults: defaults).issues
  }

  static func importTOML(_ source: String, defaults: UserDefaults = .standard) -> LiteScreenConfigurationImportResult {
    ...
    guard !preparedImport.issues.contains(where: { $0.severity == .error }) else {
      return LiteScreenConfigurationImportResult(appliedChangeCount: 0, issues: preparedImport.issues)
    }
    ...
    return LiteScreenConfigurationImportResult(
  ...
  private static func prepareImport(_ source: String, defaults: UserDefaults) -> PreparedImport {
    ...
    do {
      ...
      return PreparedImport(
        issues: [LiteScreenConfigurationIssue(severity: .error, message: error.localizedDescription)],
        mutations: []
      )
    }

    var reader = LiteScreenConfigurationReader(document: document)
    var mutations: [() -> Void] = []
  ...
  private static func validateSchema(_ reader: inout LiteScreenConfigurationReader) {
    guard let schemaVersion = reader.int("schema_version") else { return }
    if schemaVersion != 1 {
      reader.error("Unsupported schema_version \(schemaVersion). Lite Screen currently supports schema_version 1.")
    }
  }

  private static func collectGeneral(
    _ reader: inout LiteScreenConfigurationReader,
  ...
  private static func collectCapture(
    ...
    collectBool(&reader, "capture", "screenshot", "include_lite_screen", mutations: &mutations) {
  ...
  private static func collectRecording(
    ...
    collectBool(&reader, "recording", "include_lite_screen", mutations: &mutations) {
  ...
  private static func collectColor(
    ...
    guard let color = LiteScreenConfigurationColor.color(from: colorHex),
  ...
  private static func expandedPath(_ path: String) -> String {
    LiteScreenConfigurationPaths.expandedUserPath(path)
  }
}
