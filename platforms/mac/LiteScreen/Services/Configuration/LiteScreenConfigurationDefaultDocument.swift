//
//  LiteScreenConfigurationDefaultDocument.swift
//  LiteScreen
//
...
@MainActor
enum LiteScreenConfigurationDefaultDocument {
  ...
  private static func writeCapture(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("include_lite_screen", false)
  ...
  private static func writeRecording(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value(
      "left_color",
      LiteScreenConfigurationColor.hexString(from: MouseHighlightConfiguration.defaultLeftHighlightColor)
    )
    writer.value(
      "right_color",
      LiteScreenConfigurationColor.hexString(from: MouseHighlightConfiguration.defaultRightHighlightColor)
    )
    ...
    writer.value("color", LiteScreenConfigurationColor.hexString(from: RecordingAnnotationPreferences.defaultColor))
  ...
  private static func writeShortcutValues(_ writer: inout SimpleTOMLWriter, shortcut: ShortcutConfig?) {
    ...
    writer.value("key", LiteScreenConfigurationShortcutCodec.exportKey(shortcut))
    writer.stringArray("modifiers", LiteScreenConfigurationShortcutCodec.exportModifiers(shortcut))
  }
