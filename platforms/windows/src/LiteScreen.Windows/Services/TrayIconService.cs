...
{
    private const string DefaultBalloonTitle = "Lite Screen";
    ...
    public TrayIconService(AppSettings settings, Func<TimeSpan>? recordingElapsed = null)
    {
        ...
        _icon = new Forms.NotifyIcon { Text = "Lite Screen", Visible = settings.ShowTrayIcon };
        try
        {
            var resource = System.Windows.Application.GetResourceStream(new Uri("pack://application:,,,/Assets/lite-screen-icon.png"));
    ...
    private Forms.ContextMenuStrip BuildMenu(AppSettings settings)
    {
        ...
        menu.Items.Add(new Forms.ToolStripSeparator());
        menu.Items.Add(LocalizationService.TranslatePhrase("退出 Lite Screen", settings.Language), null, (_, _) => ExitRequested?.Invoke(this, EventArgs.Empty));
        return menu;
    }
    ...
    private void UpdateTooltip()
    {
        ...
        var duration = _settings.ShowRecordingDurationInTray && _recordingElapsed is not null
            ...
            : string.Empty;
        var tooltip = $"Lite Screen · {status}{duration}";
