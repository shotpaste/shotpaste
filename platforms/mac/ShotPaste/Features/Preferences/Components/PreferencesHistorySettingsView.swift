...
struct HistorySettingsView: View {
    ...
    Form {
      ...
      Section(L10n.PreferencesHistory.displaySection) {
        SettingRow(
          ...
          Picker("", selection: $manager.defaultFilter) {
            Text(L10n.CaptureKind.screenshot).tag(CaptureHistoryCategory?.some(.screenshot))
            Text(L10n.Actions.scrollingCapture).tag(CaptureHistoryCategory?.some(.scrollingScreenshot))
            Text(L10n.CaptureKind.recording).tag(CaptureHistoryCategory?.some(.recording))
            Text(L10n.OneShot.clipboard).tag(CaptureHistoryCategory?.some(.clipboard))
          }
          .labelsHidden()
          .pickerStyle(.menu)
          .fixedSize()
      ...
      Section(L10n.PreferencesHistory.storageSection) {
        SettingRow(
          ...
          title: L10n.PreferencesHistory.storageSection,
  ...
  private func revealMediaClipboardStorage() {
    guard let url = storageManager.mediaClipboardDirectoryURL else { return }
