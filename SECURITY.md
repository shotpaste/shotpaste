...
- Windows 翻译及文字整理仅向用户配置的 LLM 地址发送文字。除 HTTPS 外，支持用户显式配置的回环及
  RFC 1918 私有 IPv4 HTTP 地址；HTTP 不提供传输加密。公网 HTTP、URL 内嵌凭据、查询参数和片段不被接受，
  请求不跟随重定向。该规则不放宽固定 ASR/TOS 服务的网络边界。
