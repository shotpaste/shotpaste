...
final class CaptureHistoryRecordTests: XCTestCase {
  ...
  func testFileURLAndFileExistsReflectStoredPath() throws {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("ShotPasteTests_HistoryRecord_\(UUID().uuidString)", isDirectory: true)
  ...
  func testThumbnailURLRequiresExistingThumbnailFile() throws {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("ShotPasteTests_HistoryThumbnail_\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: directory) }
  ...
  func testTextRecordReadsPayloadAndUsesFirstLineAsTitle() throws {
    let fileURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("ShotPasteTests_TextHistory_\(UUID().uuidString).txt")
