...
namespace LiteScreen.Windows.InlineE2E;
...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            var executable = Path.GetFullPath(args.FirstOrDefault() ?? throw new ArgumentException(
                "Pass the LiteScreen.exe path as the first argument."));
            if (!File.Exists(executable)) throw new FileNotFoundException("LiteScreen executable was not found.", executable);
        ...
        bool drawAnnotation = false)
    {
        ...
        }) ?? throw new InvalidOperationException("Failed to start LiteScreen.");
    ...
    private static void VerifyClipboardBusyRecovery(int processId, string evidencePath)
    {
        ...
        }) { IsBackground = true, Name = "LiteScreen E2E clipboard lock" };
