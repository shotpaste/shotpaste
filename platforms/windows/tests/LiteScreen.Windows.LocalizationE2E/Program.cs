...
namespace LiteScreen.Windows.LocalizationE2E;
...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            if (args.Length < 1) throw new ArgumentException("Usage: LocalizationE2E <LiteScreen.exe> [output-root]");
    ...
    private static async Task<object> VerifyHistoryAndSettingsAsync(string executable, string root, string language)
    {
        ...
        try
        {
            ...
            var expectedSettingsTitle = LocalizationService.TranslatePhrase("Lite Screen · 设置", language);
