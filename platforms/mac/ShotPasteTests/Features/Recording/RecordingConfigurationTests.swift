...
final class RecordingConfigurationTests: XCTestCase {
  ...
  func testMouseHighlightConfiguration_defaults() {
    ...
    XCTAssertTrue(config.leftHighlightColor.isEqual(MouseHighlightConfiguration.defaultLeftHighlightColor))
    XCTAssertTrue(config.rightHighlightColor.isEqual(MouseHighlightConfiguration.defaultRightHighlightColor))
  }
  ...
  func testMouseHighlightConfiguration_usesPersistedValues() throws {
    ...
    let rightColor = NSColor(calibratedRed: 0.1, green: 0.7, blue: 0.3, alpha: 1.0)
    let rightColorData = try NSKeyedArchiver.archivedData(
      withRootObject: rightColor,
      requiringSecureCoding: true
    )
    defaults.set(colorData, forKey: PreferencesKeys.mouseHighlightLeftColor)
    defaults.set(rightColorData, forKey: PreferencesKeys.mouseHighlightRightColor)
    ...
    XCTAssertEqual(config.holdCircleSize, 75)
    XCTAssertEqual(config.animationDuration, 1.2)
    XCTAssertEqual(config.rippleCount, 5)
    XCTAssertEqual(config.highlightOpacity, 0.8)
    XCTAssertTrue(config.leftHighlightColor.isEqual(color))
    XCTAssertTrue(config.rightHighlightColor.isEqual(rightColor))
  }
  ...
  func testKeystrokeOverlayConfiguration_defaults() {
    ...
    XCTAssertEqual(config.visibility, KeystrokeOverlayConfiguration.defaultVisibility)
  ...
  func testKeystrokeOverlayConfiguration_usesPersistedValues() {
    ...
    defaults.set(KeystrokeOverlayVisibility.shortcutsOnly.rawValue, forKey: PreferencesKeys.keystrokeVisibility)
    ...
    XCTAssertEqual(config.visibility, .shortcutsOnly)
  }
