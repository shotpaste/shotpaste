...
struct CaptureContext: Equatable {
  ...
  static func fromFrontmostApp() -> CaptureContext {
    ...
    // Don't resolve LiteScreen itself as the app name
...
enum CaptureOutputKind {
  ...
  var defaultTemplate: String {
    switch self {
    case .screenshot:
      "Lite Screen_{datetime}_{ms}"
    case .recording:
      "Lite Screen_Recording_{datetime}"
    }
  }
...
enum CaptureOutputNaming {
  ...
  private static func fallbackName(for kind: CaptureOutputKind, date: Date) -> String {
    switch kind {
    case .screenshot:
      "Lite Screen_\(format(date, style: "yyyy-MM-dd_HH-mm-ss-SSS"))"
    case .recording:
      "Lite Screen_Recording_\(format(date, style: "yyyy-MM-dd_HH-mm-ss"))"
    }
  }
}
