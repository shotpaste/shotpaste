...
private let logger = Logger(subsystem: "Lite Screen", category: "HistoryThumbnailGenerator")
...
final class HistoryThumbnailGenerator {
  ...
  private let workerQueue = DispatchQueue(
    label: "litescreen.history-thumbnail-generator.worker",
    qos: .utility,
    attributes: .concurrent
  )
  private let thumbnailsDirectoryURL: URL
  private let stateQueue = DispatchQueue(label: "litescreen.history-thumbnail-generator.state")
    ...
    for url in contents where url.lastPathComponent.hasPrefix(prefix) {
      ...
      do {
        ...
        DiagnosticLogger.shared.logError(
          .history,
          error,
          "History thumbnail stale cache delete failed",
          context: ["fileName": url.lastPathComponent]
        )
      }
    }
