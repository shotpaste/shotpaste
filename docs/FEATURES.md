...
- 取消时恢复或移除捕获窗口及临时状态。
- 两个平台提供翻译入口：本地 OCR 后仅向用户配置的 LLM 发送识别文字，支持全屏或选区、源/目标语言、复制和取消。
  Windows 当前在冻结选区界面的结果卡片中展示译文；macOS 另有原位翻译排版。
...
- 支持手动和自动滚动。
- 提供实时预览与增量帧处理。
- macOS 长图预览按屏幕可用空间等比显示最新增长部分，避免长图持续缩窄；
  预览裁切不影响保存图片的完整内容与捕获像素。
- macOS 大选区的开始按钮须保持在屏幕可用区域内，并避开顶部模式切换条；
  选区外空间不足时放到选区内，展开帮助后仍可操作。
- 支持重叠匹配、重复帧拒绝、方向保护、固定边缘处理、恢复匹配和尾帧收口。
- 固定页头、页尾在成品外沿各保留一次；识别固定边缘不得删除首帧已选内容，
  也不得使首次有效拼接后的图片高度变小。首尾边缘计入输出高度上限。
- 输出一张长图，复用普通截图的输出操作。
...
- 两个平台的 AI 功能分组包含 LLM 供应商、AI 转写、AI 翻译；Agent Mode 当前仅在 macOS 提供。
  Windows 翻译与转写整理复用同一 LLM 配置，支持基础地址和完整接口地址；地址、模型及密钥分别显式保存。
  支持用户配置的 HTTPS 供应商，以及回环或 RFC 1918 私有 IPv4 地址的 HTTP 服务；完整兼容接口路径原样保留。
  AI 翻译可配置内置/自定义提示词、5–120 秒请求超时和识别文字发送开关。
  自动录屏转写、整理与语音语言在录制准备面板选择，设置页仅配置供应商账户。
...
- Windows 在 AI 功能设置中分别配置云凭据与文字模型。凭据及录制账户快照使用当前用户 DPAPI。
