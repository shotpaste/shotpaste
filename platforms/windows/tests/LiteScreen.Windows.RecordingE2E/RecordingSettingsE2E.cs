...
internal static class RecordingSettingsE2E
{
    internal static async Task<object> RunAsync(string executable, string outputRoot)
    {
        ...
        }) ?? throw new InvalidOperationException("Could not launch LiteScreen settings E2E.");
        ...
        try
        {
            ...
            var include = await WaitForAsync(product.Id, "IncludeLiteScreenInRecording");
            ...
            if (saved.RecordingClickRadius != 52 || Math.Abs(saved.RecordingClickOpacity - 0.85) > 0.001 ||
                ...
                saved.RecordingAnnotationFadeMilliseconds != 480 || !saved.IncludeLiteScreenInRecording)
            ...
            return new
            {
                ...
                saved.IncludeLiteScreenInRecording
            };
        }
        finally
        {
