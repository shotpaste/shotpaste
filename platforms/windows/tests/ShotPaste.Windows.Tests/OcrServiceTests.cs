...
public sealed class OcrServiceTests
{
    ...
    public async Task RecognizeAsync_ReadsHighContrastPrintedText()
    {
        ...
        using (var font = new System.Drawing.Font("Arial", 48, System.Drawing.FontStyle.Bold))
        {
            ...
            graphics.DrawString("SHOTPASTE OCR 12345", font, System.Drawing.Brushes.Black, 20, 45);
        }
    ...
    public async Task RecognizeAsync_IncludesQrPayload()
    {
        ...
        using var bitmap = writer.Write("https://shotpaste.local/test");
        var text = await new OcrService().RecognizeAsync(bitmap);
        Assert.Contains("https://shotpaste.local/test", text);
    }
    ...
    public async Task RecognizeDetailedAsync_MergesOcrAndMultipleQrPayloads()
    {
        ...
        using var first = writer.Write("https://shotpaste.local/one");
        using var second = writer.Write("https://shotpaste.local/two");
        ...
        Assert.Contains("https://shotpaste.local/one", result.QrPayloads);
        Assert.Contains("https://shotpaste.local/two", result.QrPayloads);
        Assert.Equal(
            ["https://shotpaste.local/one", "https://shotpaste.local/two"],
            result.QrPayloads);
        Assert.Contains("123", result.Text);
        Assert.Contains("https://shotpaste.local/one", result.Text);
        Assert.Contains("https://shotpaste.local/two", result.Text);
        Assert.Equal(2, result.Links.Count(link => link.StartsWith("https://shotpaste.local/", StringComparison.Ordinal)));
    }
    ...
    public async Task RecognizeDetailedAsync_RunsOcrAndQrConcurrentlyAndKeepsResultOrder()
    {
        ...
        Assert.Equal(
            ["https://shotpaste.local/left", "dd@example.com"],
            result.QrPayloads);
        Assert.True(result.Text.IndexOf("OCR FIRST", StringComparison.Ordinal) <
                    result.Text.IndexOf("https://shotpaste.local/left", StringComparison.Ordinal));
        Assert.True(result.Text.IndexOf("https://shotpaste.local/left", StringComparison.Ordinal) <
                    result.Text.IndexOf("dd@example.com", StringComparison.Ordinal));
        Assert.Equal(
            ["https://shotpaste.local/text", "https://shotpaste.local/left", "dd@example.com"],
            result.Links);
    }
    ...
    public async Task RecognizeDetailedAsync_UsesQrWhenOcrEngineIsUnavailable()
    {
        var service = new OcrService(
            null,
            _ => Task.FromException<string?>(new NotSupportedException("OCR language pack unavailable")),
            _ => ["https://shotpaste.local/qr-only"]);
        using var bitmap = new System.Drawing.Bitmap(32, 32);

        var result = await service.RecognizeDetailedAsync(bitmap);

        Assert.Equal("https://shotpaste.local/qr-only", result.Text);
        Assert.Equal(["https://shotpaste.local/qr-only"], result.Links);
    }
