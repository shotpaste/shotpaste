...
final class ScreenCaptureViewModel: ObservableObject, KeyboardShortcutDelegate {
  ...
  final class HiddenWindowSession {
    ...
    func restore() {
      ...
      DiagnosticLogger.shared.log(.debug, .ui, "Hidden Lite Screen windows restored", context: [
  ...
  private func hideVisibleNormalWindowsIfNeeded(_ shouldHide: Bool) -> HiddenWindowSession {
    ...
    DiagnosticLogger.shared.log(.debug, .ui, "Lite Screen windows hidden for capture", context: [
  ...
  private func prepareInlineAreaAnnotateFrozenSession(
    ...
    // used when the user excludes LiteScreen from the One Shot frozen backdrop.
