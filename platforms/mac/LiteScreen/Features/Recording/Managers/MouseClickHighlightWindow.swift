...
final class MouseClickHighlightWindow: NSWindow {
  ...
  func showClickEffect(at screenPoint: NSPoint, button: MouseClickButton = .left) {
    guard let contentView else { return }

    let viewPoint = viewPosition(from: screenPoint)
    let highlightColor = config.highlightColor(for: button)
    ...
    for i in 0 ..< count {
      ...
      let ripple = RippleRingView(center: viewPoint, configuration: config, color: highlightColor)
    ...
    let hold = HoldCircleView(center: viewPoint, configuration: config, color: highlightColor)
...
private final class RippleRingView: NSView {
  ...
  init(center: NSPoint, configuration: MouseHighlightConfiguration, color: NSColor) {
    maxDiameter = configuration.highlightSize
    ringWidth = configuration.ringWidth
    duration = configuration.animationDuration
    strokeColor = color.withAlphaComponent(configuration.highlightOpacity).cgColor
...
private final class HoldCircleView: NSView {
  ...
  init(center: NSPoint, configuration: MouseHighlightConfiguration, color: NSColor) {
    ...
    setupRingLayer(
      ringWidth: ringWidth,
      color: color.withAlphaComponent(configuration.highlightOpacity)
    )
  }
