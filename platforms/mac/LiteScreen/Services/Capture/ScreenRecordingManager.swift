...
enum RecordingAudioCompatibilityExporter {
  ...
  private static func writeNormalizedFile(
    ...
    try await withCheckedThrowingContinuation { continuation in
      let workerQueue = DispatchQueue(label: "com.ahtcfg24.litescreen.recording.audio-compatibility", qos: .utility)
      workerQueue.async {
        do {
  ...
  private static func copySamples(
    ...
    for (label, output, input) in outputsAndInputs {
      group.enter()
      let queue = DispatchQueue(label: "com.ahtcfg24.litescreen.recording.audio-compatibility.\(label)")
      var didFinish = false
...
final class ScreenRecordingManager: NSObject, ObservableObject {
  ...
  private let videoProcessingQueue = DispatchQueue(
    label: "com.ahtcfg24.litescreen.recording.video",
    qos: .userInitiated
  )
  private let audioProcessingQueue = DispatchQueue(
    label: "com.ahtcfg24.litescreen.recording.audio",
    qos: .userInteractive
  )
  private let microphoneProcessingQueue = DispatchQueue(
    label: "com.ahtcfg24.litescreen.recording.microphone",
    qos: .userInteractive
  )
